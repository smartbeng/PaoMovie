...
public class FilterLayoutUtils {
	...
	public void init() {
		// btn_Favourite = (ImageView) ((Activity)
		// mContext).findViewById(R.id.btn_camera_favourite);
		//
	...
	public void init(View view) {
		// btn_Favourite = (ImageView)
		// view.findViewById(R.id.btn_camera_favourite);
		// btn_Favourite.setOnClickListener(btn_Favourite_listener);
	...
	private void initFilterInfos() {
		...
		int[] lll = new int[] {MagicFilterType.NONE,
...
//			int ll = lll[i];
//			filterInfo = new FilterInfo();
//			filterInfo.setFilterType(MagicFilterType.NONE + ll);
//			for (int j = 0; j < favouriteFilterInfos.size(); j++) {
//				if (MagicFilterType.NONE + ll == favouriteFilterInfos.get(j).getFilterType()) {
//					filterInfo.setFavourite(true);
//					break;
//				}
//			}
...
//		}
		...
		public void onClick(View v) {
			if (position != 0 && filterInfos.get(position).getFilterType() != -1) {
				...
				if (filterInfos.get(position).isFavourite()) {
					...
					mAdapter.notifyItemRangeChanged(favouriteFilterInfos.size() + 1,
	...
	private void saveFavourite() {
		SharedPreferences shared = ((Activity) mContext).getSharedPreferences("favourite_filter",
