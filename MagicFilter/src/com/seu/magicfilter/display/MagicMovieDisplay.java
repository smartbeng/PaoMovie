...
import com.seu.magicfilter.utils.SaveTaskMovie;
import com.seu.magicfilter.utils.SaveTaskMovie.onMovieSaveListener;
/*用于视频转换*/
public class MagicMovieDisplay extends MagicDisplay{
	...
	private SaveTaskMovie movieSaveTask;
    public MagicMovieDisplay(Context context, GLSurfaceView glSurfaceView,Object lockObj){
	...
	protected void onGetBitmapFromGL(Bitmap bitmap){
		mOriginBitmap = bitmap;
		if(mIsSaving){
			movieSaveTask.execute(mOriginBitmap);
			mIsSaving = false;
		}else{
			mMagicSDK.storeBitmap(mOriginBitmap, false);
		}
	...
	public void savaImage(ByteArrayOutputStream output, onMovieSaveListener listener){
		
		movieSaveTask = new SaveTaskMovie(mContext, output, listener);
		mIsSaving = true;
		if(mFilters != null)
			getBitmapFromGL(mOriginBitmap, false);
		else
